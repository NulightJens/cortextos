...
- **Last commit:** `74374c5` — "merge: sync upstream cortextOS (108 commits) into SOMA" (merge of `27bf8e3` + upstream `a89cee2`)
- **Latest event — upstream sync (2026-05-31):** First sync since the 2026-04-23 fork. 108 upstream commits merged: external persistent crons (daemon-owned scheduler + dashboard workflows UI), ~11 daemon stability fixes, dashboard auth-bypass fix (#547), codex-app-server runtime parity, telegram multi-user + whisper, hooks framework. 7 conflicts resolved preserving SOMA's rename/brand + ADR-010 monochrome; upstream function adopted. Root + dashboard `tsc` clean; 1379 tests pass; 12 failures are pre-existing `.cortextos`→`.soma` rename debt (not merge-caused). Details in PROJECT_SOMA.md §13 (2026-05-31). ⚠️ Newly-merged crons subsystem overlaps Phase-1 minions/queue — confirm coexistence before building further.
